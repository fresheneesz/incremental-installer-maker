...
function runScripts(options) {
    ...
    // load state
    var stateFile = options.stateFile(args)
    if(!absoluteOrBackADirectory(stateFile)) {
        throw Error('The stateFile path ('+stateFile+') may not be inside the temporary package folder (because that folder will be deleted)."'
                    +' Make it either an absolute path, or start with "../"')
    }


    if(fs.existsSync(stateFile)) {
        var state = parseInt(fs.readFileSync(stateFile))
    } else {
        var state = 0
    }

    var lastFuture = Future(true)
    options.scripts.forEach(function(v, n) {
        lastFuture = lastFuture.then(function() {
            if(typeof(v) === 'function') {
                if(state <= n) {
                    var returnValue = v(args) // return value should be a future (or undefined)
                    state = n+1
                    return returnValue
                }
            } else if(typeof(v) === 'object') {
                return v.check(args).then(function(needsToRun) {
                    if(needsToRun) {
                        return v.install(args)
                    }
                })
            } else {
                // ignore it
    ...
    lastFuture.then(function() {
        // save state
        makePath(path.dirname(stateFile))
        fs.writeFileSync(stateFile, state)
    }).done() // ensure errors are visible
}
...
function build(filepath, options) {
    ...
    Future.all(packageFutures).then(function() {
        // package files with script
        return packageFiles().then(function(encodedData) {
            ...
            fs.writeFileSync(filepath, '#!/bin/bash\n'
                ...
                +'}\n'
                +'\n'
                // include incremental-installer in the package for now instead of installing on the fly (via npm)
                //+'# install incremental installer\n'
